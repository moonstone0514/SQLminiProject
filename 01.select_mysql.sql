...
select * from emp where month(hiredate) < 7;
select * from emp where month(hiredate) > 7;
...
(sal*12 + IFNULL(comm, 0) - 8800)*0.24 + 1536 AS 산출세액
FROM emp
...
-- 과세 표준이 7000만원
-- 8800~ 15000 : 1536
select ename as 이름,sal*12 as 연봉, (sal*12 - 8800) as 초과금액 ,(1536 +  (((sal*12)-8800) *0.35)) as 세금 
from emp where sal*12 between 8800 and 15000;
...
select hiredate from emp;
select hiredate from emp where hiredate = '81/09/28';
select hiredate from emp where hiredate = '1981/09/28';
select hiredate from emp where hiredate = '1981-09-28';
select hiredate from emp where hiredate = '81-09-28';
